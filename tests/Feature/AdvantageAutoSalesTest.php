...
final class AdvantageAutoSalesTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        config([
            ...
            'site.name' => 'Advantage Auto Sales',
            'site.domain' => 'advantageautosales-pa.com',
            'site.city' => 'Uniontown',
            'site.state' => 'PA',
        ]);
    }
    ...
    public function test_advantage_branding_is_shared_and_unknown_pages_return_not_found(): void
    {
        $this->get('/')
            ->assertInertia(fn (Assert $page) => $page
                ->where('site.name', 'Advantage Auto Sales')
                ->where('site.domain', 'advantageautosales-pa.com')
                ->where('site.city', 'Uniontown'));
    ...
    public function test_legacy_demo_vehicle_paths_resolve_to_public_images(): void
    {
        $vehicle = $this->vehicle([
            'main_image' => 'vehicles/2021-toyota-camry-se-a1001/large/main.webp',
        ]);

        $this->assertStringEndsWith('/images/vehicles/2021-toyota-camry-se-a1001/large/main.webp', $vehicle->main_image_url);
        $this->assertStringEndsWith('/images/vehicles/2021-toyota-camry-se-a1001/thumb/main.webp', $vehicle->main_image_thumb_url);
        $this->assertStringEndsWith('/images/vehicles/2021-toyota-camry-se-a1001/medium/main.webp', $vehicle->main_image_medium_url);
    }
    ...
    {
        config(['site.domain' => 'advantageautosales-pa.com']);
        $vehicle = $this->vehicle();
        $this->get('/')->assertInertia(fn (Assert $page) => $page->missing('reviews')->has('featuredVehicles', 1));
        $this->get('/sitemap.xml')->assertSee('https://advantageautosales-pa.com/inventory/'.$vehicle->slug, false);
        $this->get('/robots.txt')->assertSee('https://advantageautosales-pa.com/sitemap.xml', false);
    }
    ...
    public function test_imported_marketing_is_hidden_without_changing_inventory(): void
    {
        ...
        $this->get('/inventory/'.$vehicle->slug)->assertInertia(fn (Assert $page) => $page
            ...
            ->where('vehicle.seo_title', '2022 Test Vehicle in Uniontown, PA'));
